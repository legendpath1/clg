...
$act=$_REQUEST['act'];
  ...
  if($act=='save'){
	...
	if (!$content){
      showmessage("中文内容不能为空!");
    }
	if (!$en_content){
      showmessage("英文内容不能为空!");
    }
      $flg = $db->update($db_prefix.'pages',$row,"id='$id'");
      if($flg){
        admin_op_log('edit','单页',$row['title']);  //写入记录

        showmessage("编辑成功！","linkus.php?Result=modify&id=$id");
       }else{
         showmessage("编辑失败！","linkus.php?Result=modify&id=$id");
       }
    }
				...
				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<a href="javascript:vod(0);" class="red showimg_a" onMouseOver="showImgDiv(1)" onMouseOut="hideImgDiv(1)">[查看图片]
							<span style="z-index:9999;" class="showimg" id="showimg_1"><img src="<?='../'.$img_url;?>" width="400"/></span>
						</a>
						&nbsp;&nbsp;<input name="pic" type="file" style="width:200px"> 或者图片网址： <input name="advimg" type="text" value="<?php echo isset($img_url)? $img_url : null;?>" style="width:300px;"><span style="color:red;">&nbsp;&nbsp;图片最佳大小 437px*292px</span>
						</td>
					</tr>
					<tr>
						<td class="EAEAF3 right td1">简介内容：</td>
						<td class="EAEAF3 left td2" style="padding:5px">
							<textarea name="content" id="content" style="width:100%; height:335px; display:none"><?PHP echo $rs['content']?></textarea>
						</td>
					</tr>
					<tr style="display:none;">
						<td class="EAEAF3 right td1"><span class="red">* </span>英文内容：</td>
						<td class="EAEAF3 left td2" style="padding:5px">
					...
					<tr>
						...
						<td class="EAEAF3 left td2"><input name="Submit" type="submit" class="button" value=" 保存 ">&nbsp;&nbsp;&nbsp;</td>
					</tr>
				</table>
			</form>
		</div>
