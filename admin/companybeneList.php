		...
		<div class="tabsPageHeader">
			<div class="tabsPageHeaderContent">
				<ul class="navTab-tab1">
					<li tabid="list" class="list"><a><span>公司福利</span></a></li>
				</ul>
			</div>       
		</div>	
		<div class="navTab-panel tabsPageContent layoutBox" >
			<div class="msgInfo left">注意：输入的内容中请不要含有非法字符（比如半角状态下的引号）→　<a href="companybeneEdit.php?Result=add">添加内容</a></div>
			<form style="display:block;" name="form1" method="post" action="companybeneList.php">
			...
			<form action="?act=del&page=<?=$page?>" method="post"  name="myform" onSubmit="javascript:return chkbox(myform);">
				...
				<table width="100%"  border="0" cellpadding="0" cellspacing="0">
					...
					if($totals)
					{
						...
						$result=$db->query("select * from ".$db_prefix."personnel ".$where." order by stor asc,id desc limit ".$startpage.",".$pagesize);
					...
					<tr class="bgc2" onMouseOver="rowOver(this)" onMouseOut="rowOut(this)">
						...
						<td height="23" class="td1"><a href="companybeneEdit.php?Result=modify&id=<?=$rs['id']; ?>&page=<?=$page?>"><?=$rs['title']?></a></td>
						...
						<td class="td2">
							<a href="companybeneEdit.php?Result=modify&id=<?=$rs['id']; ?>&page=<?=$page?>" title="编辑"><img src="images/icon_edit.gif" border=0></a>&nbsp;&nbsp;&nbsp;
							<a href="javascript:deldata('companybeneList',<?=$rs['id'];?>,'<?=$page?>')" title="删除"><img src="images/icon_drop.gif" border=0></a>
						</td>
					</tr>
					 <? }?>
				</table>
				<table width="100%"  border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td class="fengye">
							...
							<?PHP
							page_list($page,$pagetotal,$pagesize,'companybeneEdit.php?keyword='.$keywords.'&page=',$totals);
							?>
							</div>
						</td>
					</tr>
