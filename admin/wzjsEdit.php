...
if($act=='edit'){
	...
	$formval = $_POST;
  	...
  	//上传图片
		if(!empty($_FILES['index_pic']['tmp_name'])){
			...
			$uploadimg->img_save_dir = ROOT_PATH.WZJS_IMG_DIR;
			if(!$uploadimg->upload_img($_FILES['index_pic'])){
				showmessage($uploadimg->errmsg);
			}else{
				$clientimg = WZJS_IMG_DIR.$uploadimg->img_new_name; //图片名称
				delete_file(ROOT_PATH.WZJS_IMG_DIR.$formval['clientimg']);
			}
		}else{
	...
	if($Result=='add'){
		...
		//if ($zlei==0){
			//showmessage("请选择类别!");
		//}
		$id = $db->insert($db_prefix."wzjs",$row);
		if($id){
			admin_op_log('add','banner',$row['title']);	//写入记录
			showmessage("添加成功",'wzjsList.php');
		}else{
			showmessage("添加失败",'wzjsEdit.php?Result=add');
		}
		...
		$flg = $db->update($db_prefix."wzjs",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','网站介绍',$row['id']);	//写入记录
			showmessage("编辑成功！","wzjsList.php?page=".$page);
		 }else{
			 showmessage("编辑失败！","wzjsEdit.php?Result=modify&id=".$id."&page=".$page);
		 }
	}
}elseif($Result=='modify'){
	
	$rs=$db->get_one("select * from ".$db_prefix."wzjs where id='$id'");
