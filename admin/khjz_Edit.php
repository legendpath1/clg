...
if (!$ad_name){
		...
		$id = $db->insert($db_prefix."khjz",$row);
		if($id){
			admin_op_log('add','banner',$row['title']);	//写入记录
			showmessage("添加成功",'khjz_List.php');
		}else{
			showmessage("添加失败",'khjz_Edit.php?Result=add');
		}
	}elseif($Result=='modify'){
		$flg = $db->update($db_prefix."khjz",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','banner',$row['title']);	//写入记录
			showmessage("编辑成功！","khjz_List.php");
		 }else{
			 showmessage("编辑失败！","khjz_Edit.php?Result=modify&id=".$id);
		 }
	}
}elseif($Result=='modify'){
	
	$rs=$db->get_one("select * from ".$db_prefix."khjz where id='$id'");
      ...
      <form action="?act=edit&Result=<?=$Result?>&id=<?=$id?>" method="post" name="form" enctype="multipart/form-data" onSubmit="return chkForm();">
		...
		<tbody>
           <tr>
			...
			<?PHP
            	$result = $db->query("select * from ".$db_prefix."khjztype order by id asc");
...
<!--         <option value="3">文字</option>-->
          ...
          <tr>
            <td class="EAEAF3 right td1">Flash优酷网址</td>
            <td class="EAEAF3 left td2">
