...
$act=$_REQUEST['act'];
  ...
  if($act=='save'){
	...
	if (!$content){
      ...
      $flg = $db->update($db_prefix.'pages',$row,'id=6');
      if($flg){
        admin_op_log('edit','单页',$row['title']);  //写入记录

        showmessage("编辑成功！","contact.php?Result=modify&id=6");
       }else{
         showmessage("编辑失败！","contact.php?Result=modify&id=6");
       }
    }

  }
  elseif($Result=='modify'){
    $rs = $db->get_one("select * from ".$db_prefix."pages where id=6");
  }
  ...
  <div id="navTab" class="tabsPage" >
        ...
        <div class="tabsPageHeaderContent">
          <ul class="navTab-tab1">
			<li tabid="list" class="list"><a><span>联系我们</span></a></li>
          </ul>
        </div>
      ...
      <form action="?act=save&Result=<?=$Result?>&id=<?=$id?>" method="post" enctype="multipart/form-data" name="form1" onSubmit="return chkForm();">
        <table width="100%" border="0" align="center" cellpadding="0">
		  ...
		  <tr style="display:none">
			...
			<textarea name="map" id="map" style="width:99%; height:300px; display:none"><?PHP echo $rs['map']?></textarea>
           </td>
          </tr>
          <tr>
