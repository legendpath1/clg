...
if($act=='save'){
    ...
    if($Result=='add'){
		...
		if($id){
			...
			showmessage("添加招聘信息成功！","personnelList.php");
		}else{
			showmessage("添加招聘信息失败！","personnelEdit.php?Result=add");
		}
			...
			if($flg){
				admin_op_log('edit','招聘信息',$row['title']);  //写入记录

				showmessage("编辑信息成功！","personnelList.php?page=".$page);
			}else{
				showmessage("编辑信息失败！","personnelEdit.php?Result=modify&id=".$id."&page=".$page);
			}
	}
		...
		<div class="navTab-panel tabsPageContent layoutBox" >
			...
			<form action="?act=save&Result=<?=$Result?>&id=<?=$id?>&page=<?=$page?>" method="post" enctype="multipart/form-data" name="form1" onSubmit="return chkForm();">
				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<tr>
						<td  class="EAEAF3 right td1"><span class="red">* </span>职位类别：</td>
						<td class="EAEAF3 left td2">
							<select name="Category_small" size="1">
								<option value="0">---请选择类别---</option>
								<option value="1" <? if ($rs['small_id']==1){echo "selected";}?> >人才观点</option>
								<option value="2" <? if ($rs['small_id']==2){echo "selected";}?> >培训体系</option>
								<option value="3" <? if ($rs['small_id']==3){echo "selected";}?> >职业规划</option>
							</select>
						</td>
					</tr>
					<tr>
