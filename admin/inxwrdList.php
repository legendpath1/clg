...
if($act=='edit')
{
	...
	$issue = $issue ? 0 : 1;
	$db->query("update ".$db_prefix."inxwrd set issue=$issue where id='$id'");
}
?>
<html>
<head>
  ...
  <div id="navTab" class="tabsPage" >
        ...
        <div class="tabsPageHeaderContent">
          <ul class="navTab-tab1">
			<li tabid="list" class="list"><a><span>热点新闻</span></a></li>
          </ul>
        </div>
        
		</div>	
		<div class="navTab-panel tabsPageContent layoutBox" >
		<div class="msgInfo left">请注意新闻排列顺序！</div>
        <table width="100%"  border="0" cellpadding="0" cellspacing="0" algin="center">
          ...
          <?
		   $totals = $db->counter($db_prefix."inxwrd",'','id');
		   if($totals>0)
		   {
			   $page = intval($_REQUEST['page'])>1 ? intval($_REQUEST['page']) : 1;
				$startpage=($page-1)*$pagesize;
				$pagetotal=ceil($totals/$pagesize);
			  $result=$db->query("select c.adv_postion, a.* from ".$db_prefix."inxwrd as a left join ".$db_prefix."advtype as c on c.id = a.catid ".$where." order by a.catid asc,a.stor asc,a.id desc limit ".$startpage.','.$pagesize);
          ...
          <tr class="bgc2" onMouseOver="rowOver(this)" onMouseOut="rowOut(this)">
            <td height="25" class="EAEAF3 left td1"><a href="inxwrdEdit.php?Result=modify&id=<?=$rs['id'];?>" class="showimg_a" style="text-decoration:none;" title="查看图片" onMouseOver="showImgDiv(<?=$i?>)" onMouseOut="hideImgDiv(<?=$i?>)">
          ...
          <tr>
			...
			<?PHP
			page_list($page,$pagetotal,$pagesize,'inxwrdList.php?page=',$totals);
			?></div></td>
          </tr>
      </table>
        <?  }
