...
if($act=='edit'){
	...
	if($Result=='add')
	{
		$db->insert($db_prefix.'advtype',$row);

		admin_op_log('add','banner分类',$row['title']);	//写入记录

		showmessage("添加成功",'advClsList.php');
	}
	elseif($Result=='modify')
	{
		$flg = $db->update($db_prefix.'advtype',$row,"id='$id'");
		if($flg){

			admin_op_log('edit','banner分类',$row['title']);	//写入记录
			showmessage("编辑成功！","advClsList.php");
		 }else{
			 showmessage("编辑失败！","advClsEdit.php?Result=modify&id=".$id);
		 }
	}
}
elseif($Result=='modify')
{
	$rs=$db->get_one("select * from ".$db_prefix."advtype where id='$id'");
}
