...
if($act=='edit')
{
	...
	$issue = $issue ? 0 : 1;
	$db->query("update ".$db_prefix."qyzx set issue=$issue where id='$id'");
}
?>
<html>
<head>
...
<div id="container">
  <div id="navTab" class="tabsPage" >
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0" algin="center">
          ...
          <?
		   $totals = $db->counter($db_prefix."qyzx",'','id');
		   if($totals>0)
		   {
			   $page = intval($_REQUEST['page'])>1 ? intval($_REQUEST['page']) : 1;
				$startpage=($page-1)*$pagesize;
				$pagetotal=ceil($totals/$pagesize);
			  $result=$db->query("select c.adv_postion, a.* from ".$db_prefix."qyzx as a left join ".$db_prefix."qyzxtype as c on c.id = a.catid ".$where." order by a.catid asc,a.stor asc,a.id desc limit ".$startpage.','.$pagesize);
			  if($db->num_rows($result)){
				  $i=1;
				  while($rs = $db->fetch_array($result)){
					$cat_name = $db->getOne('select title from '.$db_prefix.'qyzxtype where id='.$rs['catid']);
		  ?>
          <tr class="bgc2" onMouseOver="rowOver(this)" onMouseOut="rowOut(this)">
            <td height="25" class="EAEAF3 left td1"><a href="qyzx_Edit.php?Result=modify&id=<?=$rs['id'];?>" class="showimg_a" style="text-decoration:none;" title="查看图片" onMouseOver="showImgDiv(<?=$i?>)" onMouseOut="hideImgDiv(<?=$i?>)">
      			<?=$rs['ad_name'];?><?php if($rs['media_type']==0){?><span class="showimg" id="showimg_<?=$i?>"><img src="<?='../'.$rs['ad_code'];?>" width="888" height="383" /></span><?php }?></a></td>
      			<td class="td1"><? if(!empty($rs['link_img'])){?><img src="/<?=$rs['link_img']?>" height="20" /><? }else{echo'&nbsp;';}?></td>
            ...
            <td class="td2">
            <a href="qyzx_Edit.php?Result=modify&id=<?=$rs['id'];?>"title="编辑"><img src="images/icon_edit.gif" border=0></a>&nbsp;&nbsp;
            </td>
          </tr>
          <?PHP
          	$i++;
          ...
          <tr>
			...
			<?PHP
			page_list($page,$pagetotal,$pagesize,'qyzx_List.php?page=',$totals);
			?></div></td>
          </tr>
      </table>
