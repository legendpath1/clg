...
if($act=='edit'){
	...
	elseif($_POST['media_type'] == '1'){
	   if ((isset($_FILES['upfile_flash']['error']) && $_FILES['upfile_flash']['error'] == 0) || (!isset($_FILES['upfile_flash']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['upfile_flash']['tmp_name'] != 'none'))
        {
			$file_name=substr($_FILES['upfile_flash']['name'],-4);
			// print_r($type);
			// exit();
            /* 检查文件类型 */
			...
			if($kzm=="flv"){
            $file_name   = ADV_IMG_DIR.$urlstr .'.flv';
			}
			
            if (!move_upload_file($source_file, $target.$file_name))
            {
                showmessage("上传Flash文件失败2e！");
            }
            else
            {
                $ad_code = $file_name;
        ...
        elseif (!empty($_POST['flash_url']))
        {
            ...
            {
				if(substr(strtolower($_POST['flash_url']), strlen($_POST['flash_url']) - 4) != '.flv')
                showmessage("上传的Flash文件格式不正确！");
            }
...
if (!$ad_name){
		...
		$id = $db->insert($db_prefix."inpage",$row);
		if($id){
			admin_op_log('add','banner',$row['title']);	//写入记录
			showmessage("添加成功",'advList.php');
		}else{
			showmessage("添加失败",'advEdit.php?Result=add');
		}
	}elseif($Result=='modify'){
		$flg = $db->update($db_prefix."inpage",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','banner',$row['title']);	//写入记录
			showmessage("编辑成功！","in_video.php?Result=modify&id=42");
		 }else{
			 showmessage("编辑失败！","in_video.php?Result=modify&id=".$id);
		 }
	}
}elseif($Result=='modify'){
	
	$rs=$db->get_one("select * from ".$db_prefix."inpage where id='$id'");
	...
	if(isset($rs['ad_code']) && $rs['ad_code']){
	  ...
	  }elseif (substr($rs['ad_code'], -3, 3) == 'flv'){
	    $flash_url = $rs['ad_code'];
	  }
	}
}
  ...
  <div id="navTab" class="tabsPage" >
        ...
        <div class="tabsPageHeaderContent">
          <ul class="navTab-tab1">
			<li tabid="list" class="list"><a><span>品牌印象内容</span></a></li>
          </ul>
        </div>
        
		</div>	
		<div class="navTab-panel tabsPageContent layoutBox" >
		<div class="msgInfo left"><? if($Result=='add')echo'添加';else echo'编辑'?>内容</div>
      ...
      <form action="?act=edit&Result=<?=$Result?>&id=<?=$id?>" method="post" name="form" enctype="multipart/form-data" onSubmit="return chkForm();">
		...
		<tbody>
           <tr style="display:none;">
			...
			<?PHP
            	$result = $db->query("select * from ".$db_prefix."advtype order by stor asc,id desc");
...
<!--         <option value="3">文字</option>-->
          ...
          <tr>
            ...
            <td class="EAEAF3 left td2">
              <input type="text" name="flash_url" value="<?php echo isset($flash_url)? $flash_url : null;?>" size="35" />&nbsp;&nbsp;最佳大小：300*255
            </td>
          </tr>
          </tbody>
