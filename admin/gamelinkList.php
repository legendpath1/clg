...
/* 	$xw_index = $xw_index ? 0 : 1;
	...
	$issue = $issue ? 0 : 1;
	$db->query("update ".$db_prefix."gamelink set issue=$issue where id='$id'");
}elseif($act=='del')
{
	$fid=implode(',',$_POST['fid']);
	$db->query("delete from ".$db_prefix."gamelink where id in($fid)");
}
		...
		<div class="tabsPageHeader">
			<div class="tabsPageHeaderContent">
				<ul class="navTab-tab1">
					<li tabid="list" class="list"><a><span>游戏页面设置</span></a></li>
				</ul>
			</div>      
		</div>	
		<div class="navTab-panel tabsPageContent layoutBox" >
			<div class="msgInfo left">注意：输入的内容中请不要含有非法字符，也不建议从网页或是word里直接复制内容到编辑器<!--  →　<a href="gamelinkEdit.php?Result=add">添加内容</a> --></div>
				<!-- <form name="form1" method="post" action="gamelinkList.php">
				...
				<form action="?act=del&page=<?=$page?>" method="post"  name="myform" onSubmit="javascript:return chkbox(myform);">
					<table width="100%"  border="0" cellpadding="0" cellspacing="0" algin="center">
						...
						$totals = $db->counter($db_prefix."gamelink",$where,'id');
						if($totals>0)
						{
							$page = intval($_REQUEST['page'])>1 ? intval($_REQUEST['page']) : 1;
							$startpage=($page-1)*$pagesize;
							$pagetotal=ceil($totals/$pagesize);
							$result=$db->get_all("select * from ".$db_prefix."gamelink".$where." order by stor asc,id desc limit ".$startpage.','.$pagesize);
						}
						foreach($result as $key=>$rs){
						?>
						<tr class="bgc2" onMouseOver="rowOver(this)" onMouseOut="rowOut(this)">
							<td height="25" class="left td1"   style="text-align:center;!important" ><a href="gamelinkEdit.php?Result=modify&id=<?=$rs['id'];?>&page=<?=$page?>" class="showimg_a" style="text-decoration:none;" title="查看"><?=$arr[$key];?></a></td>   
												
							<td class="td1"><?=$rs['stor'];?></td>
							
							<td class="td1"><?=date('Y-m-d H:i:s',$rs['addtime']);?></td>
							<td class="td2">
							<a href="gamelinkEdit.php?Result=modify&id=<?=$rs['id'];?>&page=<?=$page?>"title="编辑"><img src="images/icon_edit.gif" border=0></a>&nbsp;&nbsp;<!-- <a href="javascript:deldata('syjc',<?=$rs['id'];?>,'<?=$page?>')" title="删除"><img src="images/icon_drop.gif" border=0> --></a>
							</td>
						</tr>
					    <?php } ?>
					</table>
