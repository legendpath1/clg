...
$ad_name = trim($_POST['ad_name']);
	...
	if(!$ad_name){
		...
		if($id){
			...
			showmessage("添加成功",'index_picList3.php');
		}else{
			showmessage("添加失败",'index_picEdit3.php?Result=add');
		}
	}elseif($Result=='modify'){
		$flg = $db->update($db_prefix."index_pic",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','banner',$row['title']);	//写入记录
			showmessage("编辑成功！","index_picList3.php?page=".$page);
		 }else{
			 showmessage("编辑失败！","index_picEdit3.php?Result=modify&id=".$id."&page=".$page);
		 }
	}
}elseif($Result=='modify'){
		...
		<div class="navTab-panel tabsPageContent layoutBox" >
			...
			<form action="?act=edit&Result=<?=$Result?>&id=<?=$id?>&page=<?=$page?>" method="post" name="form" enctype="multipart/form-data" onSubmit="return chkForm();">
				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
					<tbody>						
						<tr>
							<td width="120" class="EAEAF3 right td1">标题：</td>
							<td class="EAEAF3 left td2"><input name="ad_name" type="text" style="width:396px" value="<?=$rs['ad_name'];?>"></td>
						</tr>						
						<tr>
							<td width="120" class="EAEAF3 right td1">内容：</td>
							<td class="EAEAF3 left td2"><textarea name="ad_alt" style="width:396px;height:72px;margin-bottom: 9px;margin-top:9px"><?=$rs['ad_alt'];?></textarea></td>

						</tr>
						<tr>
							<td width="120" class="EAEAF3 right td1">链接地址：</td>
							<td class="EAEAF3 left td2"><input name="ad_url" type="text" style="width:396px" value="<?=$rs['ad_url'];?>"></td>
						</tr>
						<tr style="display:none;">
							<td width="120" class="EAEAF3 right td1">手机 OR PC：</td>
							<td class="EAEAF3 left td2"><input name="catid" type="text" style="width:355px" value="15"></td>
						</tr>
					</tbody>
					<tbody id="0" <?php if($rs['media_type']!=0){echo'style="display:none;"';}?>>
						<tr>
							...
							&nbsp;&nbsp;
							<?php  } ?>
							<input name="pic" type="file" style="width:200px"> 或者图片网址： <input name="advimg" type="text" value="<?php echo isset($img_url)? $img_url : null;?>" style="width:300px;"><span style="color:red;">&nbsp;&nbsp;图片最佳大小 1742px*490px</span>
							</td>
						</tr>
					</tbody>
					<tbody id="1" <?php if($rs['media_type']!=1){echo'style="display:none;"';}?>>
						...
						<tr>
							...
							<select name="stor" size="1" onChange="changeselect1(this.value)">
								...
								<option value="1" <? if ($rs['stor']==1){echo "selected";}?> >录播系统</option>
								<option value="2" <? if ($rs['stor']==2){echo "selected";}?> >图像跟踪系统</option>
								<option value="3" <? if ($rs['stor']==3){echo "selected";}?> >庭审/审讯系统</option>
								<option value="4" <? if ($rs['stor']==4){echo "selected";}?> >视频应用软件</option>
