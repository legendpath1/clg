		...
		<div class="tabsPageHeader">
			<div class="tabsPageHeaderContent">
				<ul class="navTab-tab1">
					<li tabid="list" class="list"><a><span>滚动图1列表</span></a></li>					
				</ul>
			</div>        
		</div>	
		<div class="navTab-panel tabsPageContent layoutBox" >
			<div class="msgInfo left">注意：必须有4个可发布的连接，否则页面出错→　<a href="index_picEdit.php?Result=add">添加图片</a></div>
			<form action="?act=del" method="post"  name="myform" onSubmit="javascript:return chkbox(myform);">
				<table width="100%"  border="0" cellpadding="0" cellspacing="0" algin="center">
					<tr class="bgc1">
						...
						<td width="10%" class="td1"><b>序号</b></td>
						<td width="12%" class="td2"><b>操作</b></td>
					</tr>
					<?
					$where=" where catid=11";
					$ttal = $db->query("select * from ".$db_prefix."index_pic where catid=11");
					...
					<tr class="bgc2" onMouseOver="rowOver(this)" onMouseOut="rowOut(this)">
						...
						<td height="23" class="paddingleft10 left td1" >
							<a href="index_picEdit.php?Result=modify&id=<?=$rs['id'];?>&page=<?=$page;?>" class="showimg_a" style="text-decoration:none;">
						...
						<!-- <td class="td1"><a href="?act=edit&id=<?=$rs['id']?>&issue=<?=$rs['issue']?>"><? echo editimg($rs['issue']);?></a></td> -->
						<td class="td2">
						<a href="index_picEdit.php?Result=modify&id=<?=$rs['id'];?>&page=<?=$page;?>"title="编辑"><img src="images/icon_edit.gif" border=0></a>&nbsp;&nbsp;<a href="javascript:deldata('index',<?=$rs['id'];?>,'<?=$page?>')" title="删除"><img src="images/icon_drop.gif" border=0></a>
						</td>
					</tr>
					<?PHP
						$i++;
					...
					<table width="100%"  border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td class="fengye" colspan="6">
								...
								<?PHP
								page_list($page,$pagetotal,$pagesize,'index_picList.php?page=',$totals);
								?>
								</div>
							</td>
						</tr>
