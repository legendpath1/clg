...
$act=$_REQUEST['act'];
  ...
  if($act=='save'){
	...
	if (!$content){
      ...
      $flg = $db->update($db_prefix.'pages',$row,'id=3');
      if($flg){
        admin_op_log('edit','单页',$row['title']);  //写入记录

        showmessage("编辑成功！","ab_qyjs.php?Result=modify&id=3");
       }else{
         showmessage("编辑失败！","ab_qyjs.php?Result=modify&id=3");
       }
    }

  }
  elseif($Result=='modify'){
    $rs = $db->get_one("select * from ".$db_prefix."pages where id=3");
  }
  ...
  <div id="navTab" class="tabsPage" >
        ...
        <div class="tabsPageHeaderContent">
          <ul class="navTab-tab1">
			<li tabid="list" class="list"><a><span>品牌定位</span></a></li>
          </ul>
        </div>
