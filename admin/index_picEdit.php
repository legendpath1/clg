...
$ad_name = trim($_POST['ad_name']);
	...
	if(!$ad_name){
		...
		if($id){
			...
			showmessage("添加成功",'index_picList.php');
		}else{
			showmessage("添加失败",'index_picEdit.php?Result=add');
		}
	}elseif($Result=='modify'){
		$flg = $db->update($db_prefix."index_pic",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','banner',$row['title']);	//写入记录
			showmessage("编辑成功！","index_picList.php?page=".$page);
		 }else{
			 showmessage("编辑失败！","index_picEdit.php?Result=modify&id=".$id."&page=".$page);
		 }
	}
}elseif($Result=='modify'){
				...
				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC">
					<tbody>						
						...
						<tr style="display:none;">
							...
							<td class="EAEAF3 left td2"><input name="catid" type="text" style="width:355px" value="11"></td>
						</tr>
					</tbody>
					<tbody id="0" <?php if($rs['media_type']!=0){echo'style="display:none;"';}?>>
						<tr>
							...
							&nbsp;&nbsp;
							<?php } ?>
							<input name="pic" type="file" style="width:200px"> 或者图片网址： <input name="advimg" type="text" value="<?php echo isset($img_url)? $img_url : null;?>" style="width:300px;"><span style="color:red;">&nbsp;&nbsp;图片最佳大小 1015px*211px</span>
							</td>
						</tr>
					</tbody>
