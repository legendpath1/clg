...
if (!$ad_name){
		...
		$id = $db->insert($db_prefix."inxwrd",$row);
		if($id){
			admin_op_log('add','banner',$row['title']);	//写入记录
			showmessage("添加成功",'inxwrdList.php');
		}else{
			showmessage("添加失败",'inxwrdEdit.php?Result=add');
		}
	}elseif($Result=='modify'){
		$flg = $db->update($db_prefix."inxwrd",$row,"id='$id'");
		if($flg){
			admin_op_log('edit','banner',$row['title']);	//写入记录
			showmessage("编辑成功！","inxwrdList.php");
		 }else{
			 showmessage("编辑失败！","inxwrdEdit.php?Result=modify&id=".$id);
		 }
	}
}elseif($Result=='modify'){
	
	$rs=$db->get_one("select * from ".$db_prefix."inxwrd where id='$id'");
        ...
        <tr>
		  ...
		  <tr>
		   ...
		   <tr>
			...
			<select onChange="showMedia(this.value)" name="media_type">
