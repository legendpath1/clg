...
if($act=='save'){
    ...
    if(!empty($_FILES['title']['tmp_name'])){
			...
			$uploadimg->img_save_dir = ROOT_PATH.SYHD_IMG_DIR;
			if(!$uploadimg->upload_img($_FILES['title'])){
				showmessage($uploadimg->errmsg);
			}else{
				$proimg_title = SYHD_IMG_DIR.$uploadimg->img_new_name; //图片名称
				delete_file(ROOT_PATH.SYHD_IMG_DIR.$formval['proimg_title']);
			}
		}else{
	...
	//上传图片 活动图片
		if(!empty($_FILES['pic']['tmp_name'])){
			...
			if(!$uploadimg->upload_img($_FILES['pic'])){
				...
				$proimg = SYHD_IMG_DIR.$uploadimg->img_new_name; //图片名称
				delete_file(ROOT_PATH.SYHD_IMG_DIR.$formval['proimg']);
			}
		}else{
    ...
    if($Result=='add'){
		//dump($row);exit;
		// if ($db->num_rows($db->query("SELECT id FROM ".$db_prefix."syhd where title='$Title' ORDER BY id desc"))>0){
		// 	showmessage("活动标题已存在!");
		// }
  //       if (!$Title){
		...
		if (!$content){
			showmessage("活动规则不能为空!");
		}

		if (!$proimg){
			showmessage("活动图片不能为空!");
		}



       $id = $db->insert($db_prefix.'syhd',$row);
       if($id){
         admin_op_log('add','首页活动',$row['title']);  //写入记录

        showmessage("添加成功！","syhdList.php");
       }else{
         showmessage("添加失败！","syhdEdit.php");
       }
		...
		$flg = $db->update($db_prefix.'syhd',$row,'id='.$id);
		if($flg){
			admin_op_log('edit','证书',$row['title']);  //写入记录

			showmessage("编辑成功！","syhdList.php?page=".$page);
		}else{
			showmessage("编辑失败！","syhdEdit.php?Result=modify&id=".$id."&page=".$page);
		}
    }

}
elseif($Result=='modify'){
    $rs = $db->get_one("select * from ".$db_prefix."syhd where id='$id'");
}
		...
		<div class="tabsPageHeader">
			<div class="tabsPageHeaderContent">
				<ul class="navTab-tab1">
					<li tabid="list" class="list"><a><span><? if(empty($rs['title']))echo '添加首页活动';else echo'编辑首页活动'?></span></a></li>
				</ul>
			</div>      
		</div>	
			...
			<form action="?act=save&Result=<?=$Result?>&id=<?=$id?>&page=<?=$page?>" method="post" enctype="multipart/form-data" name="form1" onSubmit="return chkForm();">
				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<?PHP } ?>&nbsp;&nbsp;<input name="title" type="file" style="width:200px"> 或者图片网址： <input name="proimg_title" type="text" style="width:300px" value="<?=$rs['title'];?>"> <font style="color:#F00">图片最佳大小：169px*28px</font>
						</td>
					</tr>


					<tr>
						<td class="EAEAF3 right td1"><span class="red">* </span>活动详情：</td>
						<td class="EAEAF3 left td2" style="padding:5px">
						<textarea name="en_title" style="width:500px; height:56px;"><?PHP echo $rs['en_title']?></textarea>
						</td>
					</tr>
					<tr>
						<td class="EAEAF3 right td1"><span class="red">* </span>活动规则：</td>
						<td class="EAEAF3 left td2" style="padding:5px">
						<textarea name="content" id="content" style="width:500px; height:61px;"><?PHP echo $rs['content']?></textarea>
						</td>
					</tr>
					<!-- <tr>
					...
					<tr>
						<td class="EAEAF3 right td1"><span class="red">* </span>活动图片：</td>
						...
						<span style="z-index:9999;" class="showimg" id="showimg_2"><img src="<?='../'.$rs['pic'];?>" width="300"/></span></a>
						<?PHP } ?>&nbsp;&nbsp;<input name="pic" type="file" style="width:200px"> 或者图片网址： <input name="proimg" type="text" style="width:300px" value="<?=$rs['pic'];?>"> <font style="color:#F00">图片最佳大小：184px*194px</font>
						</td>
					</tr>	
